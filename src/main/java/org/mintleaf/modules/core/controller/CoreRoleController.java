...
import org.mintleaf.modules.core.domain.*;
import org.mintleaf.modules.core.service.*;
import org.mintleaf.modules.core.vo.CoreRoleVO;
...
public class CoreRoleController {
    ...
    @Autowired
    CoreRoleMenuService coreRoleMenuService;

    @Autowired
    CoreRoleButtonService coreRoleButtonService;
    ...
    public ResultMsg add(CoreRole coreRole) {
        coreRole.setCreatetime(new Date());
        coreRoleService.save(coreRole);
        return ResultMsg.ok(coreRole);
    }
    ...
    public ResultMsg delete(CoreRole coreRole) {
        coreRoleService.deleteById(coreRole.getId());
        return ResultMsg.ok();
    }
    ...
    public ResultMsg deleteBatch(String id) {
        ...
        coreRoleService.deleteBatch(idsList);
        return ResultMsg.ok();
    }
    ...
    public ResultMsg update(CoreRole coreRole) {
        ...
        coreRoleService.updateByPrimaryKeySelective(coreRole);
        return ResultMsg.ok(coreRole);
    }
    ...
    public ResultMsg updateCoreMenu(CoreRole coreRole) throws Exception{
        CoreRole menu=coreRoleService.getCoreRoleById(coreRole.getId());;
    ...
    public ResultMsg all(){
        List<CoreRole> coreMenus =coreRoleService.findAll("");
    ...
    @ResponseBody
    public ResultMsg page(@ModelAttribute CoreRoleVO vo, int pageNum, int pageSize){
        PageHelper.startPage(pageNum, pageSize);
        List<CoreRole> coreRoles =coreRoleService.findAll(vo.getName());
        PageInfo<CoreRole> pageInfo = new PageInfo<>(coreRoles);
    ...
    public ResultMsg addPermission(String menuIds,String roleId){
        ...
        coreRoleMenuService.deleteCoreMenuByRoleId(Integer.valueOf(roleId));
        //根据id删除角色按钮关系表所有与roleid关联的数据
        coreRoleButtonService.deleteSample(Integer.valueOf(roleId));

        String[] ids = menuIds.split(",");
        for (String id : ids) {
            CoreMenu end=coreMenuService.getCoreMenuById(Integer.parseInt(id));
            //判断是否为菜单id
            if(end!=null) {
                //向角色菜单关系表插入角色新修改的菜单权限
                rm.setRoleid(Integer.valueOf(roleId));
                rm.setMenuid(Integer.valueOf(id));
                coreRoleMenuService.save(rm);
            }else{
                //向角色按钮关系表插入角色新修改的菜单权限
                rb.setRoleid(Integer.valueOf(roleId));
                rb.setButtonid(Integer.valueOf(id));
                coreRoleButtonService.save(rb);
            }
        }
        return  ok();
    }
    ...
    public ResultMsg getPermission(String roleId){
        try {
            //根据角色id获取菜单权限
            List <CoreRoleMenu>coreRoleMenu=coreRoleMenuService.getCoreRoleMenuByRoleId(Integer.valueOf(roleId));
            //根据角色id获取按钮权限
            List <CoreRoleButton>coreRoleButton=coreRoleButtonService.getCoreRoleButtonByRoleId(Integer.valueOf(roleId));
            //合并序列
    ...
    public ResultMsg getMenuTree(){
        List<CoreMenu> listMenu =coreMenuService.findAll("");
        List<CoreButton> listButton =coreButtonService.findAll();
